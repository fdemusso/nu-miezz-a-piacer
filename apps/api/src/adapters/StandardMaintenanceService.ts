...
export class StandardMaintenanceService implements IMaintenanceService {
  async openReport(report: VehicleConditionReport): Promise<void> { }

  async getVehiclesNeedingMaintenance(): Promise<Vehicle[]> { return [] }

  async getReports(): Promise<VehicleConditionReport[]> { return [] }

  async markVehicleOutOfService(vehicleId: string, reason: string): Promise<void> { }
}
